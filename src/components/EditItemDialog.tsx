...
export interface EditItemDialogProps {
  item: MenuItem;
  onEditItem: (id: string, updatedItem: {
    name: string;
    category: string;
    price: number;
    status: string;
...
const EditItemDialog: React.FC<EditItemDialogProps> = ({ item, onEditItem }) => {
  ...
  useEffect(() => {
    if (item) {
      ...
      // Set preview image if item has an image
      if (item.image) {
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <button className="text-kifome-muted hover:text-kifome-primary p-1 mr-2">
          <Pencil size={18} />
        </button>
      </DialogTrigger>
      <DialogContent className="bg-kifome-background border-kifome-border">
        <DialogHeader>
          <DialogTitle className="text-kifome-text">Editar Item</DialogTitle>
        </DialogHeader>
